...
<head>
    ...
    <style>
        ...
        .accordion-content.open {{
            max-height: 999999px; /* Huge limit to prevent text cut-off */
            opacity: 1;
            padding-top: 1rem;
            padding-bottom: 1rem;
        }}
...
def parse_markdown_to_accordions(md_content):
    ...
    # To capture main document title
    doc_title = None

    # First pass to find main title and handle blockquotes for solutions
    processed_lines = []
    in_solution = False
    ...
    for line in lines:
        if line.startswith('# ') and not doc_title:
            ...
            # Don't add to processed_lines so it doesn't appear in body text
            continue

        # Detect solution blockquotes
        if line.strip().startswith('> **SOLUCIÓN') or line.strip().startswith('> **Solución') or line.strip().startswith('> **SOLUCION'):
            in_solution = True
            processed_lines.append('<details class="mt-4 mb-6 bg-gray-50 p-4 rounded-lg border border-gray-200"><summary class="font-bold text-[#008037] cursor-pointer hover:text-[#00A859]">Ver Solución</summary><div class="mt-4">\n\n' + line + '\n')
            continue

        if in_solution:
            if not line.startswith('>') and line.strip() != '':
                # End of solution blockquote
                in_solution = False
...
def process_files():
    ...
    for md_file in md_files:
        ...
        if match:
            ...
            # Now parse_markdown_to_accordions returns a tuple
            doc_title, accordions = parse_markdown_to_accordions(content)
            html_content = generate_html_content(accordions)

            title = doc_title if doc_title else f"{type_id.capitalize()} {ra_id}"
